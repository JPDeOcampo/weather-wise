...
const Search = () => {
  ...
  return (
    <BoxContainer className="w-full flex gap-2">
      <input
        type="text"
        disabled={(city.length === 0) & loading}
        className="bg-transparent w-full h-full text-neutral-white outline-transparent border-0"
      ...
      {(isClearVisible || searchQuery.length > 10) && (
        <button
          className="z-10 text-lg text-neutral-purple hover:text-neutral-purple80"
          onClick={handleClear}
        >
          <IoMdCloseCircle />
        </button>
